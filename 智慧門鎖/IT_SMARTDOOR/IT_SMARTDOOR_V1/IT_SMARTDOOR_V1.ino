...
char json[100];
// (*) Wifi AP 連線設定
const char* ssid = "MakerClub";     //"CTAS_information04";   // 04 或 06
const char* password = "206206206"; //"24071400";            // CTAS Fax#
...
void connectAndSubscribe() {
  if (mqttclient.connect( UID, USERNAME, PASSWORD)) {
     ...
     delay(100);
     mqttclient.subscribe(TOPIC_ANY_LED,1);
     mqttclient.subscribe(TOPIC_MEMBER,1);
  }
  else {
...
void loop() {
  ...
  for(int i = 0; i < memcount; i++){
    ...
    else if(content != member[i] && i == (memcount - 1)){
      Serial.println(content);
      Serial.println(" 不合法卡...");
      Serial.println();
